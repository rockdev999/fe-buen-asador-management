...
import { IngredientKardexTypeEnum } from "@/constants";
import { IngredientStock } from "../../../models/ingredient.model";
import { useIngredientKardexHandler } from "../../../hooks/useIngredientStock";
import { INGREDIENT_KARDEX_REASON_LABELS } from "../../../config/ingredient.table";

interface IngredientKardexModalProps {
  stockItem: IngredientStock;
  onClose: () => void;
}

export const IngredientKardexModal = ({
  stockItem,
  onClose,
}: IngredientKardexModalProps) => {
  const { data: movements, status } = useIngredientKardexHandler(
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center gap-3 py-10 text-center">
            ...
            <div>
              ...
              <p className="mt-0.5 text-[11px] text-muted-foreground">
                Las compras, mermas y consumos de este insumo aparecerán
                aquí.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2">
            {movements!.map((movement) => {
              const isEntry = movement.type === IngredientKardexTypeEnum.ENTRY;
              return (
                <div
                  key={movement.id}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] text-muted-foreground/60">
                        · {INGREDIENT_KARDEX_REASON_LABELS[movement.reason]}
                      </span>
                    </div>
                    <p className="text-[11px] text-muted-foreground">
                      ...
                      {movement.unitCost != null &&
                        ` · ${formatMoney(movement.unitCost)}/u`}
                    </p>
                  </div>
