...
import {
  useIngredientStockAlertsHandler,
  useResolveIngredientStockAlert,
} from "../../../hooks/useIngredientStock";

interface IngredientStockAlertsModalProps {
  locationId: string;
  onClose: () => void;
}

export const IngredientStockAlertsModal = ({
  locationId,
  onClose,
}: IngredientStockAlertsModalProps) => {
  const [showResolved, setShowResolved] = useState(false);

  const { data: alerts, status } = useIngredientStockAlertsHandler(
    locationId,
    showResolved,
  );
  const {
  ...
  return (
    <AppModal size="lg" maxHeightClassName="max-h-[85vh]" contentClassName="max-w-lg">
      <PanelHeader
        icon={AlertTriangle}
        title="Alertas de stock de insumos"
        description="Insumos por debajo del stock mínimo establecido."
        onClose={onClose}
      />
              ...
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-muted-foreground">
                  {showResolved
                    ...
                    : "Todos los insumos están dentro de su stock mínimo."}
                </p>
              </div>
            </div>
          ) : (
