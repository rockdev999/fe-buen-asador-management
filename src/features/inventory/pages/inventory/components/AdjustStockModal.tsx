...
import { AdjustStockFormConfig } from "../../../forms/inventory.form-config";
import { AdjustStockForm } from "../../../validators/inventory.schema";
import { ADJUSTMENT_TYPE_LABELS, STOCK_UNIT_LABELS } from "../../../config/inventory.table";
...
export const AdjustStockModal = ({
  ...
  const { mutate: adjustStock, status: adjustStatus } = useAdjustStock(
  ...
  } = useFormik<AdjustStockForm>({
    initialValues: {
      ...AdjustStockFormConfig.initialValues,
      productId: stockItem.product.id,
    },
    validate: AdjustStockFormConfig.validationSchemaCreate,
    onSubmit: (data) => {
      const dto = AdjustStockFormConfig.mapFormToDTO(data);
      adjustStock(dto, { onSuccess });
    },
  });

  const previewQuantity =
    formValues.adjustmentType === AdjustmentType.ADD
              ...
              <div>
                ...
                <p className="text-[15px] font-semibold text-inkblack tabular-nums">
                  ...
                  <span className="text-[11px] font-normal text-muted-foreground">
                    {STOCK_UNIT_LABELS[stockItem.unitOfMeasure]}
                  </span>
                </p>
              </div>
              <div>
            ...
            <div className="flex items-start gap-2 rounded-xl border border-blue-100 bg-blue-50/60 px-3 py-2.5">
              ...
              <p className="text-[11px] leading-4 text-blue-700">
                ...
                <span className="font-semibold">
                  {previewQuantity} {STOCK_UNIT_LABELS[stockItem.unitOfMeasure]}
                </span>
              </p>
            </div>
